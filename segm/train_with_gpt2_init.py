"""
Example script showing how to train a Segmenter model with the MaskTransformer decoder 
initialized from a pretrained GPT-2 model.
"""
...
@click.option("--dataset", type=str, default="ade20k", help="dataset name")
...
@click.option("--gpt2-model-name", default="gpt2", type=str, 
              help="GPT-2 model name: 'gpt2', 'gpt2-medium', 'gpt2-large', or 'gpt2-xl'")
...
def main(
    ...
    if "mask_transformer" in decoder:
        ...
        # Add GPT-2 initialization config
        decoder_cfg["use_gpt2_init"] = True
    ...
    print("Creating model with GPT-2 initialized decoder...")
    model = create_segmenter(net_kwargs)
    print(f"Model created with {num_params(model)} parameters")
    model.to(ptu.device)
    ...
    if resume and checkpoint_path.exists():
        ...
        if amp and "loss_scaler" in checkpoint:
            loss_scaler.load_state_dict(checkpoint["loss_scaler"])
        start_epoch = checkpoint["epoch"] + 1
        num_iterations = checkpoint["num_iterations"]
        variant["algorithm_kwargs"]["start_epoch"] = start_epoch
    else:
        start_epoch = 0

    # Save variant
    variant_str = yaml.dump(variant)
    Path(f"{log_dir}/variant.yml").write_text(variant_str)
